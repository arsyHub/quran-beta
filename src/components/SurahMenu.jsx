...
export default function SurahMenu() {
  ...
  const searchSurah = (event) => {
    ...
    if (keyword !== "") {
      const filteredSurah = originalDataSurah?.filter((surah) =>
  ...
  return (
    <div>
      <div className="bg-[#f7f7f7] w-[300px] h-screen hidden md:flex flex-col items-center pt-20 ">
        <label className="w-[280px] py-3 input input-bordered flex items-center gap-2">
          ...
          <svg
            ...
            viewBox="0 0 16 16"
            fill="currentColor"
            className="h-7 w-7 opacity-70 text-[#00957D]"
          >
            <path
              fillRule="evenodd"
        ...
        <div className="rounded-md pt-5 mt-10 overflow-y-scroll w-full flex flex-col items-center">
          {dataSurah?.map((surah) => (
            <div
              ...
              key={surah.nomor}
              className={
                surah.nomor === activeItem
                  ? " bg-[white] cursor-pointer shadow-md p-5 rounded-lg w-[270px] flex  gap-3 border border-[#00957D] mb-4"
                  : "bg-[white] cursor-pointer p-5 rounded-lg w-[270px] flex gap-3  mb-4"
              }
            >
              <div className=" relative w-[30px] h-[33px] flex items-center justify-center">
                <Image
                  className="absolute left-0 top-0"
                  src="/img/ic-frame-number.svg"
                  alt="Logo"
                  width={30}
                  height={30}
                ></Image>
              ...
              <div>
                <div>
                  <h1 className="text-[#00957D] font-bold">
                    {surah.namaLatin}
                  </h1>
                  <p className=" text-[12px]">{surah.arti}</p>
                  <p className="text-[#00957D] text-[12px]">
